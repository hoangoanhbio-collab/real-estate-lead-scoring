...
# Custom CSS cho giao diện TPBank Style (Deep Purple & Vibrant Gold/Orange)
st.markdown("""
<style>
    ...
    * {
    ...
    /* Giao diện nền tối TPBank */
    .stApp {
        background: radial-gradient(circle at top right, #3A105C, #18032B);
        color: #ffffff;
    }
    ...
    section[data-testid="stSidebar"] {
        ...
        border-right: 1px solid #4a156d;
    }
    
    /* Tiêu đề lớn */
    .main-title {
        background: linear-gradient(135deg, #ffffff 0%, #f3a465 50%, #ff8a00 100%);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        font-weight: 800;
        font-size: 2.8rem;
        margin-bottom: 0.2rem;
        text-shadow: 0 10px 20px rgba(255, 138, 0, 0.15);
    }
    ...
    /* Nút bấm của hành động nhanh (Màu cam TPBank) */
    div.stButton > button {
        background: linear-gradient(135deg, #ff8a00 0%, #ea5f0c 100%);
        color: white;
        border: none;
        padding: 10px 20px;
        font-weight: 600;
        border-radius: 8px;
        box-shadow: 0 4px 15px rgba(234, 95, 12, 0.2);
        transition: all 0.2s ease;
    }
    
    div.stButton > button:hover {
        background: linear-gradient(135deg, #ff9f29 0%, #ff6b00 100%);
        box-shadow: 0 6px 20px rgba(234, 95, 12, 0.4);
        transform: translateY(-1px);
    }
    
    /* Hộp thông tin chỉ số (Metric Box) Glassmorphism màu tím đậm TPBank */
    .metric-card {
        background: rgba(74, 21, 109, 0.3);
    ...
    .crm-panel {
        background: rgba(74, 21, 109, 0.35);
        backdrop-filter: blur(12px);
        -webkit-backdrop-filter: blur(12px);
...
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/e/e8/TPBank_logo.png", width=120)
...
st.sidebar.divider()
with st.sidebar.expander("📋 Bảng Tổng Kết Kiểm Tra (Audit Checklist)"):
    st.markdown("""
    | Thành Phần | Công Cụ | Mô Tả |
    | :--- | :--- | :--- |
    | **1. Input** | Google Sheets | 500 khách hàng BĐS |
    | **2. Agent** | Logic chấm điểm | Tự động quét mô tả |
    | **3. Tools** | Streamlit, Pandas, Git | Nền tảng xây dựng |
...
# Sử dụng st.columns(3) kết hợp st.metric theo đúng slide hướng dẫn
col_m1, col_m2, col_m3 = st.columns(3)
with col_m1:
    st.metric(label="Tổng khách hàng 🏠", value=f"{total_leads} lead", help="Tổng số lead nạp vào")
with col_m2:
    st.metric(label="Khách VIP (Nóng 🔥) (+50đ)", value=f"{hot_count} lead", delta="Tiềm năng cao")
with col_m3:
    st.metric(label="Khách Rác (Rác 🗑️) (-50đ)", value=f"{trash_count} lead", delta="-50đ", delta_color="inverse")

# Phê duyệt statistics dạng nhỏ hơn bên dưới
st.write("")
col_sub1, col_sub2, col_sub3 = st.columns(3)
with col_sub1:
    st.metric(label="Đã phê duyệt ✅", value=f"{approved_count} lead")
with col_sub2:
    st.metric(label="Đã loại bỏ ❌", value=f"{rejected_count} lead")
with col_sub3:
    st.metric(label="Chờ kiểm duyệt ⏳", value=f"{pending_count} lead")
...
if len(filtered_df) > 0:
    ...
    st.markdown(f"""
    <div class="crm-panel">
        <h4 style="color:#ff8a00;margin-bottom:10px;margin-top:0;">🏢 Khách hàng: {customer_data['Họ và tên']} ({customer_data['Số điện thoại']})</h4>
